 ...
 * 停止任务命令
 * 对应API: stopTask
 * 用法: aid stopTask --TaskID xxx [--config ./config/config.properties]
 */
    ...
    public void execute(String[] args) {
        ...
        try {
            ...
            if (cmd.hasOption("help")) {
                new HelpFormatter().printHelp("aid stopTask", options, true);
                return;
            }
            ...
            try (AidClient client = new AidClient(configPath)) {
                AidResponse<TaskStatusResponse> response = client.stopTask(taskId);
