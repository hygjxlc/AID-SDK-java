 ...
 * 校验任务文件命令
 * 对应API: newTaskverify
 * 用法: aid newTaskverify --TaskID xxx [--config ./config/config.properties]
 */
public class NewTaskverifyCommand extends BaseCommand {
    ...
    public void execute(String[] args) {
        ...
        try {
            ...
            if (cmd.hasOption("help")) {
                new HelpFormatter().printHelp("aid newTaskverify", options, true);
                return;
            }
            ...
            try (AidClient client = new AidClient(configPath)) {
                AidResponse<VerifyResponse> response = client.newTaskverify(taskId);
