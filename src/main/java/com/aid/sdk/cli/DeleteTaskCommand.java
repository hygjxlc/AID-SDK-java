 ...
 * 删除任务命令
 * 对应API: deleteTask
 * 用法: aid deleteTask --TaskID xxx [--config ./config/config.properties]
 */
    ...
    public void execute(String[] args) {
        ...
        try {
            ...
            if (cmd.hasOption("help")) {
                new HelpFormatter().printHelp("aid deleteTask", options, true);
                return;
            }
            ...
            try (AidClient client = new AidClient(configPath)) {
                AidResponse<TaskStatusResponse> response = client.deleteTask(taskId);
