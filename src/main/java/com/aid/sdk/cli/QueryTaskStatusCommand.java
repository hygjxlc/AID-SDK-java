...
/**
 * 查询任务状态命令
 * 对应API: queryTaskStatus
 * 用法: aid queryTaskStatus --TaskID xxx [--config ./config/config.properties]
 */
public class QueryTaskStatusCommand extends BaseCommand {
    ...
    public void execute(String[] args) {
        ...
        try {
            ...
            if (cmd.hasOption("help")) {
                new HelpFormatter().printHelp("aid queryTaskStatus", options, true);
                return;
            }
            ...
            try (AidClient client = new AidClient(configPath)) {
                AidResponse<TaskStatusResponse> response = client.queryTaskStatus(taskId);
