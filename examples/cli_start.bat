...
REM 检查 JAR 文件位置（开发环境、安装包环境、或嵌套解压环境）
if exist "%SDK_DIR%\target\aid-client-sdk-1.0.0.jar" (
    set "JAR_PATH=%SDK_DIR%\target\aid-client-sdk-1.0.0.jar"
) else if exist "%SDK_DIR%\aid-client-sdk-1.0.0.jar" (
    set "JAR_PATH=%SDK_DIR%\aid-client-sdk-1.0.0.jar"
) else if exist "%SDK_DIR%\..\aid-client-sdk-1.0.0.jar" (
    REM 处理嵌套解压的情况: aid-client-sdk-1.0.0/aid-client-sdk-1.0.0/examples/
    set "JAR_PATH=%SDK_DIR%\..\aid-client-sdk-1.0.0.jar"
) else (
    echo [错误] 找不到 SDK JAR 文件
    echo 请确保以下位置之一存在 JAR 文件:
    echo   - %SDK_DIR%\target\aid-client-sdk-1.0.0.jar      (开发环境)
